...
def main():
    ...
    all_parameters_num = sum([param.nelement() for name, param in model.named_parameters()])
    
    from transformers.adapters import PrefixTuningConfig

    config = PrefixTuningConfig(flat=False, prefix_length=args.prefix_length, bottleneck_size=300)
    model.add_adapter("prefix_tuning", config=config)
    
    # reduction_factor = int((768*768*2*12)/(all_parameters_num*args.adapter_size*0.01))
    # adapter_config = transformers.AdapterConfig(original_ln_before= True, original_ln_after= True, residual_before_ln= True, adapter_residual_before_ln= False, ln_before= False, ln_after= False, mh_adapter= False, output_adapter= True, non_linearity= 'relu', \
    ...
    allow_optimize_list = ['prefix_tuning']
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(dev_dataloader):
            label_position_idx = batch.pop('label_position_idx')
            outputs = model(**batch)
            # predictions, labels = extract_predict_label(label_position_idx, outputs.logits, label_word_map)
    ...
    if args.do_predict:
        ...
        for step, batch in enumerate(test_dataloader):
            label_position_idx = batch.pop('label_position_idx')
            outputs = model(**batch)
            prediction = extract_predict_label(label_position_idx, outputs.logits, label_word_map).numpy().tolist()
            predictions.append(prediction)

        with open(args.output_predict_file, 'w') as writer:
...
def add_prompt(task_name, example, sentence1_key, sentence2_key, mask_token='<mask>'):
    ...
    if task_name=='sst2':
        ...
        # return [' '.join([item, 'It', 'was', mask_token, '.']) for item in sentence1_list]
...
def extract_real_label(label_position_idx, output_logits, labels, label_word_map):
    ...
    # 因为现在的label也是label word形式的，所以虽然叫做labels，但也不是真正能用的label
