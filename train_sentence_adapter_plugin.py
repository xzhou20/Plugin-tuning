...
def parse_args():
    ...
    # 3 or 0.1
    parser.add_argument(
        "--adapter_size", type=float, default=0.1)
...
def main():
    ...
    def preprocess_function(examples):
        ...
        # if "label" in examples:
        #     label_word_list = [label_word_map[item] for item in examples["label"]]
        #     if label_to_id is not None:
...
# def add_prompt_ont_hot(task_name, examples, tokenizer, mask_token='<mask>'):
#     for i in range(len(examples['input_ids'])):
#         start_idx = 0
#         for j, mask_index in enumerate(examples['input_ids'][i]):
#             if mask_index != 1:
#                 start_idx=j
#                 break
